...
class BaseTrainerS(object):
    def __init__(self, args, model, loss_func, score_func, train_loader, dev_loader, use_multi_gpu=True):
        ...
        if not os.path.exists(self.args.ckpt_root):
    ...
    def init_trainner(self, resume_from=None, use_summary_writer=True):
        ...
        if resume_from is None:
            self.optim = t.optim.Adam([i for i in self.model.parameters() if i.requires_grad is True], lr=1e-3,
                                      weight_decay=1e-4)
            self.scheduler = t.optim.lr_scheduler.ReduceLROnPlateau(self.optim, 'max', 0.5, 0, verbose=True, cooldown=0,
                                                                    min_lr=1e-7)
        else:
    ...
    def load(self, load_from_exp):
        ...
        self.scheduler = trainner_state['scheduler']
...
"""
    ...
    def train_epoch(self):
        for data in tqdm(self.train_loader, desc='train step'):
            ...
            train_loss.backward()
            if self.use_multi_gpu:
                self.model.module.word_embedding.weight.grad.data[0] = 0
            else:
                self.model.word_embedding.weight.grad.data[0] = 0
    ...
    def train_inference(self, data):
        feature, caption = [i.cuda() for i in data]
        self.optim.zero_grad()
        token_id, prob_vector = self.model(feature, caption)
        loss = self.loss_func(prob_vector, caption)
        return loss
